...
void autoLogic() {
  ...
  if(lowTower == true) { //Move lift to to score low tower
    lift.movePos(-2400, 900); //Moves lift based on encoder value
    lowTower = false;

  }

  if(highTower == true) { //Move lift to to score high tower
    lift.movePos(-3000, 900); //Moves lift based on encoder value
    highTower = false;

  }

  if(slowOutTake == true) { //Outtake specific amount to stack cubes
  ...
  if(autoTopCube == true) { //Gets top cube during competition autonomous
  ...
  if(resetLift == true) { //Moves lift down
    LiftM = 127; //Simply time based, no need for any fancy encoder values
    pros::delay(2000);
    LiftM = 0;
    resetLift = false;
  ...
  if(autoTrayUp == true) { //Moves tray to specific encoder position
    tray.movePos(1700, 1500); //Moves based on encoder values
    autoTrayUp = false;

  }

  if(autoTrayDown == true) {
    TrayM = 127; //Simply time based, no need for any fancy encoder values
    pros::delay(1000);
    TrayM = 0;
    autoTrayDown = false;
...
void compLogic() {
  if(slowOutTake == true) { //Slow outtake function that preps for stacking
    Intake1.tare_position(); //Reset encoders
    Intake2.tare_position();
  ...
  if(dunk == true) { //Dunks cubes into the towers
    Intake1.tare_position(); //Reset encoders
    Intake2.tare_position();
  ...
  if(lightS == true) { //Outtakes cubes to cover light sensor mounted on the tray
  ...
  if(resetAngler == true) { //Resets tray
  ...
  if(liftDown == true) { //Account for the variation in limit switch position
    liftDown = false;
    newLiftDown = false;
